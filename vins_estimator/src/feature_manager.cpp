...
{
    for (int i = 0; i < NUM_OF_CAM; i++)// 所有帧数
        ric[i].setIdentity();// 初始化为单位矩阵
}
...
/**
 * @brief   把特征点放入feature的list容器中，计算每一个点跟踪次数 和 它在次新帧和次次新帧间的视差，返回是否是关键帧
 * @param[in]   frame_count 窗口内帧的个数
 * @param[in]   image 某帧所有特征点的[camera_id,[x,y,z,u,v,vx,vy]]s构成的map,索引为feature_id，camera_id为可观测到特征点的帧
...
void FeatureManager::removeBack()
{
    ...
    {
        it_next++;
        // 1.如果特征点起始帧号start_frame不为零则首次观测的图像帧号减一
        if (it->start_frame != 0)
            it->start_frame--;
        // 2.如果start_frame为0则直接移除feature_per_frame的第0帧FeaturePerFrame
        // 3.如果feature_per_frame为空则直接删除特征点
        else
        {
            it->feature_per_frame.erase(it->feature_per_frame.begin());

			  ...
			  vector<SFMFeature> &sfm_f, map<int, Vector3d> &sfm_tracked_points)
{
	...
	//2、PnP得到l,l+1,l+2...frmaenum-2相机位姿，三角化l,l+1,l+2...frmaenum-2和framenum-1帧
	...
	//3、l和l+1,l+2......framenum-2三角化
	for (int i = l + 1; i < frame_num - 1; i++)
		triangulateTwoFrames(l, Pose[l], i, Pose[i], sfm_f);


	// 4、pnp得到l-1,l-2.....0相机位姿，l-1, l-2, l-3, …, 0帧与l帧 三角化
	for (int i = l - 1; i >= 0; i--)
	{
		//solve pnp
		Matrix3d R_initial = c_Rotation[i + 1];
	...
	for (int j = 0; j < feature_num; j++)
	{
		...
		if ((int)sfm_f[j].observation.size() >= 2)
		{
			...
			int frame_0 = sfm_f[j].observation[0].first;// 第一帧
			point0 = sfm_f[j].observation[0].second;
			int frame_1 = sfm_f[j].observation.back().first;// 最后一帧
